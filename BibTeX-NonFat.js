{
	"translatorID":"d82b7fc9-713c-4bf8-afce-0cf8c57d132c",
	"translatorType":3,
	"label":"BibTeX (Non-Fat)",
...
/// BibTeX-Nonfat.js  ---  minimal BibTeX export for Zotero
// 
...
// These fields get mapped between Zotero and LaTeX with no modification
// Their format is   latexfield:"zoteroField"
var fieldMap = {
	address:"place",
	chapter:"section",
	edition:"edition",
...
// This is the main routine for Exporting BibTeX keys
// Several of the stanzas in this section can be neatly commented out
// to prevent those fields from being exported.
...
function doExport() {
	...
	while(item = Zotero.nextItem()) {
		...
		if(item.creators && item.creators.length) {
	        ...
	        // modfied: we don't want the 'month' exported if it's a journal and it has an 
                // issue number (Cf. CMS 17.161)
		if(item.date) {
			var date = Zotero.Utilities.strToDate(item.date);
			// need to use non-localized abbreviation
			if (!((item.itemType == "journalArticle") && (item.issue))) {
